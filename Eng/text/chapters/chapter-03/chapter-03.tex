...
Given a dataset $\mathcal{D} = \{(\mathbf{x}_i, \mathbf{y}_i)\}_{i=1}^{N}$ and a model $f(\mathbf{x}; \boldsymbol{\theta})$, the empirical risk is:
\begin{equation}
\mathcal{R}(\boldsymbol{\theta}) = \frac{1}{N}\sum_{i=1}^{N} \ell(f(\mathbf{x}_i; \boldsymbol{\theta}), \mathbf{y}_i)
\end{equation}
