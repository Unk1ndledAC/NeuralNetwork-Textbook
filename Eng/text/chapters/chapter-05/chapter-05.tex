...
\end{equation}
This matches a $15 \times 15$ standard convolution but with only $3 \times 9C^2$ parameters instead of $225C^2$. This is the core idea behind \textbf{DeepLab} \cite{chen2017deeplab}.
\end{example}
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption[Max pooling 2x2, stride 2]{Max pooling $2\times2$, stride 2}
    \label{fig:maxpool}
\end{figure}
...
Group Normalization (GN) divides channels into $G$ groups and normalizes within each group independently \cite{wu2018group}. This provides a unified framework that subsumes LayerNorm and InstanceNorm as special cases.
...
Dropout \cite{srivastava2014dropout} randomly sets activations to zero during training with probability $p$:
\begin{equation}
...
Dropout can be viewed as approximate Bayesian inference \cite{gal2016dropout}. Each dropout mask defines a different sub-network, and training with dropout is equivalent to:
\begin{equation}
...
\textbf{DropPath} (Stochastic Depth) \cite{huang2016deep} randomly skips entire residual blocks during training. For a block with input $\mathbf{x}$ and output $\mathcal{F}(\mathbf{x})$:
\begin{equation}
...
\begin{itemize}
    ...
    \item \cite{ioffe2015batch} --- Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift
    \item \cite{ba2016layer} --- Layer Normalization
    \item \cite{gal2016dropout} --- Dropout as a Bayesian Approximation
    \item \cite{huang2016deep} --- Deep Networks with Stochastic Depth
    \item \cite{chen2017deeplab} --- Rethinking Atrous Convolution for Semantic Image Segmentation
    \item \cite{howard2017mobilenets} --- MobileNets: Efficient CNNs for Mobile Vision Applications
    \item \cite{wu2018group} --- Group Normalization
\end{itemize}

\section{Exercises}

\begin{enumerate}
    \item \textbf{Convolution Output Size:} Compute the output height and width for an input of size $32\times32$, kernel size $5$, padding $2$, stride $1$, and dilation $1$. Repeat for stride $2$.

    \item \textbf{Depthwise Separable FLOPs:} For input shape $56\times56\times64$ and output channels $128$, compare the FLOPs of a standard $3\times3$ convolution with a depthwise separable convolution.

    \item \textbf{Dilated Receptive Field:} Three $3\times3$ convolutions use dilation rates $1, 2, 4$. Derive the effective receptive field and explain why this is useful in semantic segmentation.

    \item \textbf{Pooling Choice:} Compare max pooling and average pooling on a feature map containing one strong activation and many weak activations. Which operation better preserves salient local evidence?

    \item \textbf{Normalization Strategy:} Your GPU memory only allows batch size 2 for a detection model. Would you choose BatchNorm, LayerNorm, or GroupNorm? Explain the trade-offs.

    \item \textbf{Dropout in Practice:} Why is dropout usually disabled at inference time? Derive why inverted dropout keeps the expected activation unchanged during training.
\end{enumerate}
