...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Learning rate range test. The steepest descent point $\alpha^*$ provides a good starting learning rate.}
...
\begin{itemize}
    ...
    \item Acts as implicit regularization (M\"uller et al., 2019 showed it is equivalent to adding KL divergence between the output distribution and a uniform prior)
...
Dynamic loss scaling (Micikevicius et al., 2018) automatically adjusts $S$: increase $S$ by a factor of 2 every $N$ successful steps, and halve $S$ whenever gradients overflow (contain NaN/Inf).
...
\begin{itemize}
    \item Goodfellow et al., ``Deep Learning'' \cite{goodfellow2016deep} --- Chapter 7 (Regularization) and Chapter 8 (Optimization for Training Deep Models)
    ...
    \item Micikevicius et al., ``Mixed Precision Training'' (ICLR 2018)
