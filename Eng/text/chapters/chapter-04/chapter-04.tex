...
While the \textbf{theoretical} receptive field covers the entire region above, the \textbf{effective} receptive field (the region that actually influences the output) has a Gaussian-like distribution centered on the receptive field center \cite{luo2016understanding}. Pixels near the boundary contribute significantly less to the output than those at the center.
...
\subsection{\texorpdfstring{$1 \times 1$}{1x1} Convolution: Cross-Channel Mixing}
...
\begin{itemize}
    \item Typical random initialization yields $\gamma \ll 1$ with high probability \cite{bengio1994learning}
...
LSTM \cite{hochreiter1997long} addresses the vanishing gradient problem with gating mechanisms.
...
GRU \cite{cho2014properties} simplifies LSTM with fewer gates while maintaining performance.
...
\begin{itemize}
    \item \cite{bengio1994learning} --- Learning Long-Term Dependencies with Gradient Descent Is Difficult
    \item \cite{hochreiter1997long} --- Long Short-Term Memory
    \item \cite{cho2014properties} --- Learning Phrase Representations Using RNN Encoder--Decoder
    \item \cite{luo2016understanding} --- Understanding the Effective Receptive Field in Deep CNNs
...
\begin{enumerate}
    \item \textbf{DNN Parameter Counting:} A fully connected network has layer widths $784 \rightarrow 256 \rightarrow 128 \rightarrow 10$. Compute the total number of trainable parameters including biases.

    \item \textbf{Receptive Field Growth:} Compute the receptive field of three stacked $3\times3$ convolutions with stride 1. How does it compare with one $7\times7$ convolution in both receptive field and parameter count?

    \item \textbf{$1\times1$ Convolution:} Show that a $1\times1$ convolution with $C_{in}$ input channels and $C_{out}$ output channels is equivalent to a position-wise fully connected layer. How many parameters does it use?

    \item \textbf{RNN Gradient Decay:} If the recurrent Jacobian norm is bounded by $\gamma = 0.92$, estimate the gradient attenuation over 50 time steps. Why does this make long-horizon learning difficult?

    \item \textbf{LSTM Gates:} Explain the role of the forget gate in maintaining long-term memory. What happens when $\mathbf{f}^{(t)} \approx 0$ and when $\mathbf{f}^{(t)} \approx 1$?

    \item \textbf{Architecture Selection:} For each of the following tasks, choose a preferred architecture and justify it: image classification, machine translation, tabular credit scoring, and multivariate time-series forecasting.
\end{enumerate}
