...
Alternative: Rotary Position Embedding (RoPE) \cite{su2021roformer} is used in LLaMA and GPT-NeoX, which encodes relative position directly into the attention computation.
...
Vision Transformer \cite{dosovitskiy2020image} applies the Transformer architecture to image classification by treating image patches as tokens:
...
GPT (Generative Pre-trained Transformer) \cite{radford2019language,brown2020language} uses a decoder-only architecture:
\begin{itemize}
...
BERT \cite{devlin2018bert} uses an encoder-only architecture with bidirectional attention:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{SwiGLU activation:} Used in LLaMA \cite{touvron2023llama}, better than GeGLU
    \item \textbf{Flash Attention:} IO-aware exact attention algorithm \cite{dao2022flashattention}, reduces memory from $O(n^2)$ to $O(n)$
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Attention Matrix Shapes:} For sequence length $n=128$ and model dimension $d_{model}=512$ with 8 heads, what are the shapes of $\mathbf{Q}$, $\mathbf{K}$, and $\mathbf{V}$ per head?

    \item \textbf{Why Scale by $\sqrt{d_k}$?} Explain what happens to the softmax distribution when dot products become very large in magnitude.

    \item \textbf{Positional Encoding:} Why must Transformers inject positional information while RNNs do not need a separate positional encoding mechanism?

    \item \textbf{ViT Tokens:} A $224\times224$ image is split into $14\times14$ patches. How many tokens does the Transformer process after adding a [CLS] token?

    \item \textbf{Encoder vs. Decoder:} Compare BERT and GPT in terms of masking pattern, architecture type, and pretraining objective.

    \item \textbf{Efficiency Trade-off:} Why can Flash Attention accelerate exact attention without changing the mathematical result of the attention operation?
\end{enumerate}
