...
\begin{itemize}
    ...
    \item \textbf{Masked Language Modeling (MLM):} Predict randomly masked tokens (BERT-style) \cite{devlin2018bert}
...
For vision:
\begin{itemize}
    \item \textbf{Contrastive Learning:} Maximize similarity between positive pairs \cite{chen2020simple}
    \item \textbf{Masked Autoencoding (MAE):} Reconstruct masked patches \cite{he2022masked}
...
LoRA \cite{hu2021lora} reduces trainable parameters by decomposing weight updates:
...
\begin{itemize}
    ...
    \item \textbf{Uncertainty weighting:} Weight by inverse task uncertainty \cite{kendall2018multi}
...
\begin{enumerate}
    ...
    \item \textbf{Elastic Weight Consolidation (EWC):} Protect important weights with regularization \cite{kirkpatrick2017overcoming}
...
Goyal et al. \cite{goyal2017accurate} achieved 256-way data parallelism on ImageNet with 8K batch size.
...
\begin{enumerate}
    \item \textbf{Fine-tuning Strategies:} Compare full fine-tuning, linear probing, and LoRA for a downstream task with limited labeled data and limited GPU memory.

    \item \textbf{LoRA Parameter Count:} A linear layer has shape $4096 \times 4096$ and LoRA rank $r=8$. Compare the number of trainable parameters in full fine-tuning vs. LoRA.

    \item \textbf{Multi-Task Weighting:} Why can equal weighting fail in multi-task learning? How does uncertainty-based weighting address this issue?

    \item \textbf{Continual Learning:} Explain the intuition behind Elastic Weight Consolidation. What role does the Fisher information matrix play?

    \item \textbf{Self-Supervised Learning:} Contrast contrastive learning with masked image modeling. What supervision signal does each method create from unlabeled data?

    \item \textbf{Large-Batch Training:} If you increase batch size by a factor of 8, what does the linear scaling rule suggest for the learning rate, and why is warmup still needed?
\end{enumerate}
