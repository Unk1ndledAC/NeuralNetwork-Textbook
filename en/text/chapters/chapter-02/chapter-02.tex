...
\begin{enumerate}
    ...
    \item \textbf{Whether deeper networks are more efficient}---Cybenko's and Hornik's results apply to single hidden layers, but depth provides \emph{exponential} efficiency gains for compositional functions \cite{montufar2014number}.
\end{enumerate}
...
Swish has become a widely used smooth alternative to ReLU in deeper networks \cite{ramachandran2017searching}.
...
Mish \cite{misra2019mish} is a self-regularized, non-monotonic activation function:
\begin{equation}
...
StarReLU, introduced in \cite{maulik2024starrelu}, is a self-gated activation function specifically designed for transformers:
\begin{equation}
...
\begin{itemize}
    ...
    \item \cite{clevert2015fast} - Fast and Accurate Deep Network Learning by Exponential Linear Units (ELUs)
    \item \cite{he2015delving} - Delving deep into rectifiers: He initialization
    \item \cite{hendrycks2016gelu} - Gaussian Error Linear Units (GELU)
    \item \cite{ramachandran2017searching} - Systematic comparison of activation functions
    ...
    \item \cite{goodfellow2016deep} - Deep Learning, Chapter 6
\end{itemize}
